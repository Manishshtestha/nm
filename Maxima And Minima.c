...
void main(){
	...
	for(i=0;i<n;i++) scanf("%f%f",&x[i],&fx[i]);
	h = x[1]-x[0];
	for(i=0;i<n;i++) fd[i]=fx[i];
	for(i=0;i<n;i++)
	{
		for(j=n-1;j>i;j--) fd[j]=(fd[j]-fd[j-1]);
	}
	a = (1/2.0)*fd[3];
	...
	if(val<0) printf("Maxima exists at x =%f\n",x1);
	else printf("Minima exists at x =%f\n",x1);
